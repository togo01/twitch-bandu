var twitchIRC = function (username, oauth, options) {
  ...
  //  ws = new WebSocket("ws://192.16.64.145/");
  ...
  //alert(location.href.indexOf(":"));
    ...
    function whisperTo (user, message) {
      ...
      //  ws.send("PRIVMSG #srwfe :/w " + user + " " + message);
        ...
        for (i in messages) {
            ...
            if (messages.hasOwnProperty(i)) {
                ...
                // Emit the "message" event
                    ...
                    fire("message", message.channel, message.user, message.message);
                    fire("message_with_tags", message.channel, message.user, message.message,message.tags);
                }
                // Emit the "message" event
                else if (message.command === "PRIVMSG" && message.action === true) {
